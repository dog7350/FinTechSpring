...
public class AutoLoginInterceptor extends HandlerInterceptorAdapter implements LoginSession {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (loginCookie != null) { // 현재 자동 로그인 쿠키가 있는지 검증
			MemberDTO dto = ms.getUserSessionId(loginCookie.getValue()); // DB에서 쿠키에 해당하는 정보 로드
			if (dto != null) {
				request.getSession().setAttribute(LOGIN, dto.getId()); // 로그인 세션 생성
			}
		}
